    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6 col-xl-6">
                <h1 class="h1-card">Role Detail</h1>
            </div>
        ...
        <div class="nav-wrapper mt--2">
            <ul class="nav nav-pills nav-fill flex-column flex-md-row" id="tabs-icons-text" role="tablist">
                <li class="nav-item">
                    <a class="nav-link mb-sm-3 mb-md-0 active" id="view-role-info-tab" data-toggle="tab" href="#view-role-info" role="tab" aria-controls="view-role-info" aria-selected="true">
                        <i class="fas fa-info-circle mr-2"></i> Detail
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link mb-sm-3 mb-md-0" id="view-role-users-tab" data-toggle="tab" href="#view-role-users" role="tab" aria-controls="view-role-users" aria-selected="false">
        ...
        <div class="card shadow">
            <div class="card-body">
                <div class="tab-content" id="myTabContent">
                    ...
                    <div class="tab-pane fade" id="view-role-users" role="tabpanel" aria-labelledby="view-role-users-tab">
                        <table class="table table-responsive-lg table-responsive-xl mt-6 mb-4" id="view-role-users-table">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Name</th>
                            ...
                            <tbody>
                                <tr v-for="item in users" :key="item.id" :id="'view-role-users-item-'+item.id">
                                    <td>{{ item.id }}</td>
                                    <td>{{ item.name }}</td>
                                    <td>{{ item.email }}</td>
                    ...
                    <div class="tab-pane fade" id="view-role-permissions" role="tabpanel" aria-labelledby="view-role-permissions-tab">
                        <table class="table table-responsive-lg table-responsive-xl mt-6 mb-4" id="view-role-permissions-table">
                            ...
                            <tbody>
                                <tr v-for="item in permissions" :key="item.id" :id="'view-role-permissions-item-'+item.id">
                                    <td>{{ item.id }}</td>
                                    <td>{{ item.name }}</td>
...
export default {
    ...
    created () {
        ...
        axios.get('/api/admin/role/' + name).then(res => {
    ...
    updated () {
        this.users_table = $('#view-role-users-table').DataTable();
        this.permissions_table = $('#view-role-permissions-table').DataTable();
    },
    methods: {
        deletePermission (id) {
            axios.delete('/api/admin/permission/' + id).then(res => {
                toastr.success(res.data.message);
                this.permissions_table.rows('#view-role-permissions-item-'+id).remove().draw(false);
            }).catch(error => {
                toastr.error(error.response.data.message);
            })
        },
        deleteUser (id) {
            axios.delete('/api/admin/user/' + id).then(res => {
                toastr.success(res.data.message);
                this.users_table.rows('#view-role-users-item-'+id).remove().draw(false);
            }).catch(error => {
                toastr.error(error.response.data.message);
            })
        }
