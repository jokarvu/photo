    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-6 col-md-6 col-sm-6 col-xl-6">
                <h1 class="h1-card">Location</h1>
            </div>
            <div class="col-lg-6 col-md-6 col-sm-6 col-xl-6 text-right">
                <router-link to="/admin/location/add" class="btn btn-md btn-primary btn-card-h1"><i class="fas fa-plus"></i> New location</router-link>
            </div>
        </div>
        <div class="card">
            <div class="card-body">
                <table class="table table-responsive-sm table-responsive-md table-responsive-lg table-responsive-xl mt-6 mb-4" id="list-locations-table">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Name</th>
                    ...
                    <tbody>
                        <tr v-for="item in locations" :key="item.id" :id="'list-locations-item-'+item.id">
                            <td>{{ item.id  }}</td>
                            <td>{{ item.name  }}</td>
                                    ...
                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                Do you really want to delete this location from database?
                                            </div>
...
export default {
    ...
    created () {
        axios.get('/api/admin/location').then(res => {
    ...
    updated () {
        this.table = $('#list-locations-table').DataTable();
    },
    methods: {
        deleteLocation (id) {
            axios.delete('/api/admin/location/' + id).then(res => {
                toastr.success(res.data.message);
                this.table.rows('#list-locations-item-'+id).remove().draw(false);
            }).catch(error => {
                toastr.error(error.response.data.message);
            })
        }
